...
/*
* @Author: cuiyi
* @Date:   2016-09-14 16:09:16
* @Last Modified by:   ElliotCui
* @Last Modified time: 2016-09-14 16:09:39
*/
...
;
(function() {
  var APK_DOWNLOAD_LINK, APP_STORE_LINK, HOME_PAGE_LINK, YING_YONG_BAO_LINK, getFailedLink, isIOS, isWeixinBrowser, school_template, tryOpenApp;

  $(document).on('submit', '#search-schools-form', function(e) {
    var $this, schools_container, search_message, search_result_hint;
    e.preventDefault();
    $this = $(this);
    schools_container = $('#schools-container');
    search_result_hint = $('#search-result-hint');
    search_message = $('#search-message');
    search_message.html('载入中...');
    search_result_hint.hide();
    schools_container.html('');
    return $.ajax({
      url: $this.attr('action'),
      dataType: 'json',
      data: $this.serializeArray(),
      beforeSend: function(xhr) {
        return xhr.setRequestHeader('Authorization', 'Basic MTAyOTM4NDc1Njpxd2VydHl1aW9w');
      },
      success: function(data) {
        var schools;
        schools = data.schools;
        if (schools.length === 1) {
          window.location = $this.data('next-page') + ("?school_id=" + schools[0].id);
          return;
        }
        if (schools.length === 0) {
          search_message.html('<span class="glyphicon glyphicon-info-sign"></span>没有找到你搜索的学校');
          return;
        }
        search_message.html('');
        schools_container.html('');
        search_result_hint.show();
        return schools.forEach(function(s) {
          return schools_container.append(school_template(s));
        });
      }
    });
  });

  school_template = function(school) {
    return $('#school-template').html().replace(/{{([a-z]*)}}/g, function(fw, w) {
      return school[w];
    });
  };
  ...
  YING_YONG_BAO_LINK = "http://a.app.qq.com/o/simple.jsp?pkgname=fm.jihua.kecheng&g_f=991668";

  APP_STORE_LINK = "http://itunes.apple.com/cn/app/ke-cheng-ji-hua/id505933342";

  APK_DOWNLOAD_LINK = "http://kecheng-files.b0.upaiyun.com/app/kecheng.apk";

  HOME_PAGE_LINK = "https://www.kechenggezi.com/";
  ...
  getFailedLink = function() {
    if (isWeixinBrowser()) {
      return YING_YONG_BAO_LINK;
    } else if (isIOS()) {
  ...
  tryOpenApp = function() {
    var $this, click_at, native_app_link;
    $this = $(this);
    native_app_link = "classbox://";
    click_at = new Date;
    setTimeout(function() {
      if (new Date - click_at < 2000) {
        return window.location = getFailedLink();
      }
    }, 1);
    return window.open(native_app_link, "_self");
  };
...
function setIndex(index, carousel) {
  if(index != currentIndex && !moving) {
    moving = true
    currentIndex = index
...
function bindPagenum(carousel) {
  $('a.pagenum').bind('click', function() {
      var index = $.jcarousel.intval($(this).attr("rel"));
      setIndex(index + 1, carousel)
      return false
  });

  $('.callout').bind('click', function() {
      var index = $.jcarousel.intval($(this).attr("rel"));
      setIndex(index + 1, carousel)
      return false
  });
}
function setActivePagenum(num) {
...
function setActiveScreenshot(num) {
  url = "url(/assets/welcome/screenshot_" + num + ".png)"
  ...
  $(".interface_container"+active).fadeOut(1200)
  $(".interface_container"+buffer).css("background", url)
  $(".interface_container"+buffer).fadeIn(1200)
}

function onItemIn(carousel, item, idx, state) {
  moving = false;
  setActivePagenum(idx);
  setActiveCallout(idx);
  url = "url(/assets/welcome/line_" + idx + ".png)"
  $("#line").css("background-image", url)
  $("#line").show();
};

function onItemOut(carousel, item, idx, state) {
};
...
$(document).ready(function() {
  ...
  $('#scroll-arrow').bind('click', function() {
  ...
  $('#downloadBtn').bind('click', function() {
    ...
    if(isiOS) {
      document.location.href = "http://itunes.apple.com/cn/app/ke-cheng-ji-hua/id505933342";
    } else {
      document.location.href = "http://files.cdn.kechenggezi.com/app/kecheng.apk";
    }
  });
  ...
  $('#slides').jcarousel({
    ...
    scroll: 1,
    animation: 800,
    initCallback: bindPagenum,
    // auto: 1,
    itemFirstInCallback: onItemIn,
    itemFirstOutCallback: onItemOut,
    buttonNextHTML: null,
    buttonPrevHTML: null,
